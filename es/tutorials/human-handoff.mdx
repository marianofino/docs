---
 ...
 description: Deriva conversaciones a un humano (human handoff) clasificándolas en una categoría y disparando un webhook.
---
...
app.post("/webhooks/human-handoff", express.json(), async (req, res) => {
  ...
  // TODO: enrutar al cliente correcto, encolar para agente humano, notificar, etc.
  res.status(204).send();
});
```

## Paso 3: Seguimiento y ruteo al cliente correcto

- Guarda cada evento del webhook y muestra una cola de conversaciones que requieren atención humana.
- Muestra en tu backoffice `name`, `phone_number`, `summary`, `reason` y `created_at`.
